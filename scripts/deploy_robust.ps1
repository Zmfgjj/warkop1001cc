...
Compress-Archive -Path frontend/dist/* -DestinationPath frontend/dist_upload.zip -Force

Write-Host "Splitting zip into chunks..."
# Use absolute path for .NET method
$absPath = Convert-Path "frontend/dist_upload.zip"
...
for ($i = 0; $i -lt $totalChunks; $i++) {
    ...
    [System.IO.File]::WriteAllBytes((Resolve-Path .).Path + "\$filename", $chunk)
...
$joinCmd = "cd /tmp && cat chunk_*.bin > dist_upload.zip && cd /var/www/frontend && rm -rf * && unzip -o /tmp/dist_upload.zip && chmod -R 755 /var/www/frontend && chown -R www-data:www-data /var/www/frontend && rm -f /var/www/landing_page/bundle.zip && zip -r /var/www/landing_page/bundle.zip . -x '*.apk' -x '*.zip' -x '*.tar' && rm -f /tmp/chunk_*.bin && echo DEPLOY_OK"
